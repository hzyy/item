...
$(function() {
    ...
    var commentPost = function(bucket, fileUrl, content) {
    	var signObj=getSignature();
        $.ajax({
            type: 'POST',
            url: hostName + '/comment/post',
            data: {
                loginUserId: signObj.loginUserId,
...
function getCommentHtml(commentId, likeCount, userId, nickName, headPhotoUrl, commentFileUrl, commentContent, stime) {
    ...
    s += '<div class="media-right" style="padding-right:10px;" onclick="commentLike(\'' + loginUser.userId + '\',\'' + commentId + '\')"><a href="javascript:;"><img src="../images/zan_hover.png" /></a><span id="like_' + commentId + '" class="likeCount" style="padding-top:4px;padding-right:3px;" class="pull-right">' + likeCount + '</span></div>';
    s += "</div>";
    return s;
}
...
function setLoginUser(userId, nickName, bucket, headPhoto) {
