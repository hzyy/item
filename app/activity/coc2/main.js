        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success == '1') {
                    var nickName = "";
                    if (loginUser.userId == '' || loginUser.userId == '0') {
                        nickName = "游客";
                    } else {
                        nickName = decodeURIComponent(loginUser.nickName);
                    }
                    var headPhotoUrl = '';
                    if (!loginUser.bucket || loginUser.bucket.length == 0 || !loginUser.headPhoto || loginUser.headPhoto.length == 0) {
                        headPhotoUrl = getOssUrl('gamecircle-pic', 'default-head-photo.jpeg');
                    } else {
                        headPhotoUrl = getOssUrl(loginUser.bucket, loginUser.headPhoto);
                    }
                    var htmls = getCommentHtml(response.commentId, 0, loginUser.userId, nickName, headPhotoUrl, getOssUrl(bucket, fileUrl), content, '刚刚');
...
function getOssUrl(bucket, url) {
    if (bucket != null && bucket.length > 0 && url.length > 0 && url != null) {
...
function commentLike(loginUserId, token, commentId) {
    $.ajax({
        type: 'POST',
...
function getCommentHtml(commentId, likeCount, userId, nickName, headPhotoUrl, commentFileUrl, commentContent, stime) {
    ...
    var s = '<div class="media border-b" style="padding-bottom:5px;background:#fff;">';
    s += '<div class="media-left"><a><img class="media-object" width="50" src="' + headPhotoUrl + '" onclick="clickURL(\'' + userId + '\')"></a></div>';
    s += '<div class="media-body"><h5 class="media-heading"><span style="color:#337ab7"  onclick="clickURL(\'' + userId + '\')">' + nickName + '</span>&nbsp;<span style="color:#999999;font-size:10px;">' + stime + '</span></h5>' + commentFileUrl + '<p style="margin:7px 0 0 0;">' + commentContent + '</p></div>';
    s += '<div class="media-right" style="padding-right: 10px;" onclick="commentLike(\'' + loginUser.userId + '\',\'' + loginUser.token + '\',\'' + commentId + '\')"><a href="javascript:;"><img src="../images/zan_hover.png" /></a><span id="like_' + commentId + '" class="likeCount" style="padding-top:4px;padding-right:3px;" class="pull-right">' + likeCount + '</span></div>';
    s += "</div>";
    return s;
}
...
function setLoginUser(userId, token, nickName, bucket, headPhoto) {
    loginUser.userId = userId;
    loginUser.token = token;
    loginUser.nickName = nickName;
    loginUser.bucket = bucket;
    loginUser.headPhoto = headPhoto;
    ...
    $(window).scroll(function() {
        ...
        if (scrollTop + windowHeight == scrollHeight) {　　
            if (pageNo <= pageCount) {　　requestComments();
            }　　
        }
    });
}