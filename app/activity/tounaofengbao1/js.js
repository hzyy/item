...
var isInit = true;
var type = 5;
var objectId = '120141814015127552';
var pageNo = 1;
var pageSize = 20;
var pageCount = 1;
var index = "";
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success == '1') {
                    ...
                    var cmindex=commentCountSpan.html();
                    if(!cmindex || cmindex == 0 || cmindex.length == 0){
                    		index = '1';
                    }else{
                    		index = parseInt(cmindex, 10) +1;
                    }
                    var htmls = getCommentHtml(index, response.commentId, 0, DeviceJS.loginUser.userId, nickName, headPhotoUrl, getOssUrl(bucket, fileUrl), content, '刚刚');
                    $('#comment').prepend(htmls);
                    
                    commentCountSpan.html(parseInt(cmindex, 10) + 1);
                    ...
                    if (response.success == '1') {
                        ...
                        if (comments && comments.length > 0) {
                            ...
                            for (var i = 0; i < comments.length; i++) {
                                ...
                                //只能看自己发布的评论
                                if (cm.userId != DeviceJS.loginUser.userId){
							    		cm.content = "不能偷看别人的答案哦!";
							    		cm.attachmentUrl = "";
							    }
                                htmls += getCommentHtml(cm.index, cm.commentId, cm.likeCount, cm.userId, nickName, headPhotoUrl, getOssUrl(cm.bucket, cm.attachmentUrl), Emoji.trans(decodeURIComponent(cm.content)), diffTime);
                            }
...
function getCommentHtml(index, commentId, likeCount, userId, nickName, headPhotoUrl, commentFileUrl, commentContent, stime) {
    ...
    s += '<div class="media-left floor">' + index + 'F</div>';
    s += '<div class="media-left" style="padding-left:10px;"><a><img class="media-object" width="50" src="' + headPhotoUrl + '" onclick="clickURL(\'' + userId + '\')"></a></div>';
